...
class Front {
            ...
            <div class="eoksp-dialog" role="dialog" aria-modal="true" aria-label="<?php echo esc_attr(get_the_title($post)); ?>">
                ...
                <div class="eoksp-dialog-head">
                    ...
                    <div class="eoksp-head-actions">
                        <span class="eoksp-meta-chip eoksp-count-chip">팝업 건수 : 총 <em><?php echo esc_html(count($slides)); ?></em>건</span>
                        <?php if (!empty($settings['hide_today']) && empty($args['preview_mode'])) : ?>
                            <button type="button" class="eoksp-meta-chip eoksp-hide-today" data-hide-today>하루동안보지않기</button>
                        <?php endif; ?>
                ...
                <div class="eoksp-body <?php echo $settings['height_mode'] === 'fixed' ? 'is-fixed-height' : 'is-auto-height'; ?>">
                    ...
                    <?php if (count($slides) > 1 && !empty($settings['show_arrows'])) : ?>
                ...
                <?php if (count($slides) > 1 && !empty($settings['show_dots'])) : ?>
    ...
    private function render_slide_content($slide, $settings) {
        ...
        if ($type === 'kboard') {
            ...
            echo '<a href="' . esc_url($data['url']) . '" class="eoksp-kboard-card ' . esc_attr($style_class) . '">';
            if (!empty($data['image'])) {
                echo '<div class="eoksp-kboard-thumb"><img src="' . esc_url($data['image']) . '" alt="' . esc_attr($data['title']) . '"></div>';
            }
            echo '<div class="eoksp-kboard-text">';
            echo '<strong class="eoksp-kboard-title">' . esc_html($data['title'] ?: 'KBoard 게시물') . '</strong>';
            echo '<p class="eoksp-kboard-excerpt">' . esc_html($data['excerpt'] ?: '게시물 내용을 불러오지 못했거나 KBoard가 비활성화되어 있습니다.') . '</p>';
